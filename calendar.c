...
struct Day create(){
    ...
    printf("Enter the date of the week: ");
    scanf("%d", &day.date);

    printf("Enter the description of the week: ");
    scanf("%s", temp);
    day.description = strdup(temp);
...
void read(struct Day *calendar , int numDays){
    for(int i = 0; i<numDays; i++){
        printf("Enter the details of the week : ");
        calendar[i] = create();
    }
}
...
void display(struct Day *calendar, int numDays){
    for(int i = 0; i<numDays; i++){
        printf("Day os the week is %s", calendar[i].name);
    }
}
